...
namespace JumboExcel
{
    ...
    {
        [Test, Explicit]
        public void DataTypeStyles()
        {
            var q =
                from applyColumns in new[] {true, false}
                from item in GetDataTypes()
                group item by item.Category
                into g
                select new Worksheet("Data Types_" + g.Key, null, g.Select(i => i.Row));

            foreach (var worksheet in q)
            {
        ...
        public void Styles()
        {
            ...
            {
                new Worksheet("Data Types", mediumColumns, GetDataTypes().Select(i => i.Row)),
                ...
                new Worksheet("Wrap text", new WorksheetParametersElement(false, false),
                    new Row(new SharedString("This is a wide string that should be wrapped.", CreateWrappedStringStyle()),
                        new SharedString("This is a wide string that should be wrapped.", CreateWrappedStringStyle()), new SharedString("This is a wide string that is not wrapped.")),
        ...
        {
            return new Row(new IntegerCell(textRotation),
                new SharedString("Rotated text", new StringStyle(null, Border.NONE, null, new Alignment(HorizontalAlignment.LEFT, VerticalAlignment.TOP, textRotation))));
        }
        ...
        private static IEnumerable<(string Category, Row Row)> GetDataTypes()
        {
        ...
        private static (string Category, Row Row) TimeSpanRow(string customTimeFormatString)
        {
        ...
        private static (string Category, Row Row) RowForType<T>(T element, string comment = "", string customCategory = "") where T : CellElement
        {
            var categoryTokens = new List<string> {typeof(T).Name};
            if (customCategory != "")
                categoryTokens.Add(customCategory);

            return (
                string.Join("_", categoryTokens),
                new Row(new InlineString(typeof(T).Name), EmptyCell.Instance, element, EmptyCell.Instance, new InlineString(comment)));
        }
